...
class Plan{
    int start,end,cores,cost;
    public Plan(int start,int end,int cores,int cost){
        this.start=start;
        this.end=end;
        this.cores=cores;
        this.cost=cost;
...
public class IBM {
    public static long getMinCost(int n,int k, List<List<Integer>>plans){
            ...
            for(Plan p:planList){
                ...
                if(i>=p.start && i<=p.end && 0<rem ){
                    if(p.cores==rem){
                        ...
                        rem=0;
                       // System.out.println(core+" "+minCost);
        ...
        for (int i = 0; i < planList.size(); i++) {
            System.out.println(planList.get(i).start+","+planList.get(i).end+","+planList.get(i).cores+","+planList.get(i).cost);
        }
        long a=minCost;
        return a;
    }
